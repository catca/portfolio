  ...
  return (
    <Container>
      {header.map(({ title, current }, index) => {
        return (
          <Nav key={index} current={current} page={currentPage} onClick={() => onClick(current)}>{title}</Nav>
        )
      })}
    </Container>
  )
...
const Nav = styled.div<containerProps>`
  ...
  color: #FFFFFF;
  cursor: pointer;
  border-bottom: ${(props: { page: any; current: any; }) => props.page === props.current ? 'solid 1px #FFFFFF' : 'none'};
`;